...
# Chemin vers les données
data_path <- here("src", "data", "TP5_a25_reg_app.txt")
data_path2 <- here("src", "data", "TP5_a25_clas_app.txt")
...
sel.aic <- stepAIC(fit,scope=y ~ .,direction="both", trace=FALSE)
formula <- formula(sel.aic)
reg <- lm(formula, data = reg_data)

X.clas = read.table("a25_clas_app.txt")
...
# `classifieur` et `regresseur` sans changer leur nom et avec
