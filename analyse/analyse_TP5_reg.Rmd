...
```
Les différentes méthodes de sélection de variables ont produit des résultats contrastés.
Le modèle de régression logistique complet présente une MSE relativement élevée, ce qui suggère un overfitting ou en tout cas une instabilité dans les prédictions.
Les méthodes basées sur les critères AIC et BIC améliorent nettement la performance : l’AIC obtient la meilleure MSE, grâce à une pénalisation plus faible qui permet de conserver davantage de variables pertinentes, tandis que le BIC, plus strict, sélectionne un modèle légèrement plus simple au prix d’une MSE un peu supérieure.
...
R² = 0.9737215
RMSE 10.06987, ce qui est plus faible que la régression linéaire avec AIC
