...
reg_data_long <- reg_data %>% dplyr::select(all_of(vars_cont)) %>% pivot_longer(cols = everything(), names_to = "variable", values_to = "valeur")
...
cor_y <- cor(reg_data %>% dplyr::select(-y), reg_data$y)
...
forward.formula <- y ~ X6 + X8 + X42 + X45 + X79 + X84 + X86 + X91
```
...
backward.formula <- y ~ X6 + X8 + X42 + X45 + X79 + X84 + X86 + X91
```
...
k=log(nrow(reg_data)), trace= FALSE)
...
model.reg.forward <- glm(forward.formula, data=reg_data_train, family="gaussian")
model.reg.backward <- glm(backward.formula, data=reg_data_train, family="gaussian")
...
cat("\n====Regression logistique avec toutes les variables====\n")
cat("MSE: ", mse.reg, "RMSE", sqrt(mse.reg), "\nformule :", "complete", "\n")
cat("\n====Regression logistique avec selection de variables par AIC====\n")
cat("MSE: ", mse.reg.aic,"RMSE", sqrt(mse.reg.aic), "\nformule ", as.character(aic.formula)[3], "\n")
cat("\n====Regression logistique avec selection de variables par BIC====\n")
cat("MSE : ", mse.reg.bic, "RMSE", sqrt(mse.reg.bic),"\nformule ", as.character(bic.formula)[3] , "\n")
cat("\n====Regression logistique avec selection de variables par méthode forward====\n")
cat("MSE : ", mse.reg.forward,"RMSE", sqrt(mse.reg.forward),"\nformule ", "y ~ X6 + X8 + X42 + X45 + X79 + X84 + X86 + X91" , "\n")
cat("\n====Regression logistique avec selection de variables par méthode backward====\n")
cat("MSE : ", mse.reg.backward,"RMSE", sqrt(mse.reg.backward), "\nformule ", "y ~ X6 + X8 + X42 + X45 + X79 + X84 + X86 + X91" , "\n")

```

Les différentes méthodes de sélection de variables ont produit des résultats contrastés. Le modèle de régression logistique complet présente une MSE relativement élevée, ce qui suggère un overfitting ou en tout cas une instabilité dans les prédictions. Les méthodes basées sur les critères AIC et BIC améliorent nettement la performance : l’AIC obtient la meilleure MSE, grâce à une pénalisation plus faible qui permet de conserver davantage de variables pertinentes, tandis que le BIC, plus strict, sélectionne un modèle légèrement plus simple au prix d’une MSE un peu supérieure.

À l’inverse, les méthodes stepwise forward et backward conduisent toutes deux à une MSE extrêmement élevée. Cela indique un sous-ajustement important : le nombre de variables retenues est trop faible pour capturer correctement la structure du problème, ce qui dégrade fortement la qualité des prédictions. Ces approches apparaissent donc inadaptées dans ce contexte.

Globalement, la sélection par AIC et BIC se révèle plus pertinente et plus stable, tandis que les approches stepwise classiques doivent être évitées ici/
...
rmse_results_aic <- numeric(k)
rmse_results_bic <- numeric(k)
...
for (i in 1:k) {
  ...
  lm_mod_aic <- lm(aic.formula, data = train_df)
  lm_mod_bic <- lm(bic.formula, data = train_df)
  
  # Prédiction
  pred <- predict(lm_mod, newdata = test_df)
  pred_aic <- predict(lm_mod_aic, newdata = test_df)
  pred_bic <- predict(lm_mod_bic, newdata = test_df)
  
  # RMSE
  rmse_results[i] <- rmse_fold(test_df$y, pred)
  rmse_results_aic[i] <- rmse_fold(test_df$y, pred_aic)
  rmse_results_bic[i] <- rmse_fold(test_df$y, pred_bic)
}
...
mean_rmse_aic <- mean(rmse_results_aic)
sd_rmse_aic <- sd(rmse_results_aic)

mean_rmse_bic <- mean(rmse_results_bic)
sd_rmse_bic <- sd(rmse_results_bic)

cat("RMSE moyen :", mean_rmse, "\n")
cat("Écart-type du RMSE :", sd_rmse, "\n")
cat("RMSE moyen avec formule AIC :", mean_rmse_aic, "\n")
cat("Écart-type du RMSE avec formule AIC :", sd_rmse_aic, "\n")
cat("RMSE moyen avec formule BIC :", mean_rmse_bic, "\n")
cat("Écart-type du RMSE avec formule BIC :", sd_rmse_bic, "\n")
```

Les résultats montrent que les modèles sélectionnés par AIC et BIC offrent un RMSE moyen légèrement inférieur et plus stable que le modèle complet, indiquant qu’une sélection de variables améliore ici modestement la précision et la robustesse de la régression linéaire.
...
```{r}
X <- as.matrix(dplyr::select(reg_data, -y))
y <- reg_data$y
...
R2 <- cor(pred, y)^2
rmse_lasso <- sqrt(mean((y - pred)^2))
cat("R² =", R2)
cat("\nRMSE", rmse_lasso)
```

LASSO utilise globalement toutes les variables (sauf X1, X4, X28, X54, X63, X65, X66, X69, X86).\
R² = 0.9737215 RMSE 10.06987, ce qui est plus faible que la régression linéaire avec AIC
...
for (i in 1:k) {
  ...
  #avec AIC
  #lm_mod_aic <- lm(aic.formula, data = train_df)
  #pred_lm_aic <- predict(lm_mod_aic, newdata = test_df)
  #results$LM_AIC[i] <- rmse_fold(y_test, pred_lm_aic)
  #results$LM_AIC[i] <- mse_fold(y_test, pred_lm_aic)
  
  #avec BIC
  #lm_mod_bic <- lm(bic.formula, data = train_df)
  #pred_lm_bic <- predict(lm_mod_bic, newdata = test_df)
  #results$LM_BIC[i] <- rmse_fold(y_test, pred_lm_bic)
  #results$LM_BIC[i] <- mse_fold(y_test, pred_lm_bic)
...
# Préparation des données
X <- reg_data %>% dplyr::select(-y) %>% as.matrix()
y <- reg_data$y
...
utiliser lasso avec x et x\^2 pour identifier les carrés d'intérêt puis mélange =\> Camille regarder tous les splines appris 1 à 1 =\> Bastien
...
```{r}
X_final <- as.matrix(df_final %>% dplyr::select(-y))
...
X_final <- scale(df_final %>% dplyr::select(-y))
...
for(i in 1:k){
  ...
  train_df <- data.frame(y = y_train, X_train)
  colnames(train_df)[-1] <- selected_vars
  
  test_df <- data.frame(X_test)
  colnames(test_df) <- selected_vars
  
  #fit avec BAM
  
  fit.bam<-bam(as.formula(final_formula),family=gaussian(),data=train_df)
  
  pred_bam <- predict.bam(fit.bam, newdata=test_df)
  
  mse_cv[i] <- mean((y_test - pred_bam)^2)
}
...
cat("\n\nMSE CV sur le modèle BAM + ALPHA/LAMBDA optis :", mean_mse, "\n")
cat("\nRMSE CV sur le modèle BAM + ALPHA/LAMBDA optis:", rmse_cv, "\n")
```
...
### Test avec AIC et regression linéaire
...
for(i in 1:k){
  ...
  lm_mod <- lm(formula, data = train_df)
  
  pred <- predict (lm_mod, newdata=test_df)
  results[i] <- mse_fold(test_df$y, pred)
}

cat("\nmean MSE", mean(as.numeric(results)))
cat("\nmean RMSE", mean(sapply(results, sqrt)))
...
alpha_res <- data.frame(alpha = alpha_grid, mse = NA, sd = NA)
...
for(j in seq_along(alpha_grid)){
  ...
  for(i in 1:k){
    ...
    el_mod <- cv.glmnet(X_train,y_train, alpha = a, nfolds = 5)
    
    pred <- predict (el_mod, newx = X_test, s="lambda.min")
    results[i] <- mse_fold(test_df$y, pred)
    fold_mses_res[i] <- mse_fold(y[test_idx], pred)
  }
  alpha_res$mse[j] <- mean(fold_mses_res)
  alpha_res$sd[j]  <- sd(fold_mses_res)
}
print(alpha_res)
best_alpha <- alpha_res$alpha[which.min(alpha_res$mse)]
...
k <- 10
folds <- sample(rep(1:k, length.out = nrow(reg_data)))

# toutes les variables (noms des colonnes de X_scaled)
vars <- colnames(X_scaled)

# Grille de tests GAM
gam_configs <- expand.grid(k = c(5, 10, 15),
                           bs = c("tp", "cr"),
                           stringsAsFactors = FALSE)

gam_res <- numeric(nrow(gam_configs))

for(cfg in seq_len(nrow(gam_configs))){
  k_s <- gam_configs$k[cfg]
  bs_s <- gam_configs$bs[cfg]
  ...
  for(i in 1:k){
    ...
    train_df <- data.frame(y = y[train_idx], X_scaled[train_idx, , drop = FALSE])
    test_df  <- data.frame(X_scaled[test_idx, , drop = FALSE])
    
    # Construire la formule avec toutes les variables
    form_str <- paste0(
      "y ~ ",
      paste0("s(", vars, ", bs='", bs_s, "', k=", k_s, ")", collapse = " + ")
    )
    
    mod_gam <- bam(as.formula(form_str),
                   data = train_df,
                   family = gaussian())
    
    pred <- predict.bam(mod_gam, newdata = test_df)
    
    fold_mses[i] <- mse_fold(y[test_idx], pred)
  }
  
  gam_res[cfg] <- mean(fold_mses)
...
df_gam <- data.frame(y = y, X_scaled[, selected_vars, drop = FALSE])
...
for(i in 1:k){
  test_idx <- which(folds == i)
  train_idx <- setdiff(1:nrow(df_gam), test_idx)
  
  train_df <- df_gam[train_idx, ]
  test_df <- df_gam[test_idx, ]
  
  mod <- bam(as.formula(form_str), data = train_df, family = gaussian())
  pred <- predict(mod, newdata = test_df)
  fold_mses[i] <- mse_fold(test_df$y, pred)
}

cat("MSE moyen CV :", mean(fold_mses), "\n")
...
form_str <- paste0("y ~ ", paste0("s(", selected_vars, ", bs='ps', k=5)", collapse = " + "))
bam_model <- bam(as.formula(form_str), data = df_gam, family = gaussian(), select=TRUE)
